...
export default function SearchDoctor() {
  ...
  // Best-effort: a doctor with no feedback yet, or a service hiccup, just
  // shows no rating badge instead of breaking the page.
  useEffect(() => {
    let ignore = false;
    if (doctors.length === 0) return;

    Promise.all(doctors.map(doc =>
      fetch(`${APPOINTMENT_API}/feedback/doctor/${doc.doctorId}`)
        .then(resp => resp.ok ? resp.json() : [])
        .then(list => ({ doctorId: doc.doctorId, list }))
        .catch(() => ({ doctorId: doc.doctorId, list: [] }))
    )).then(results => {
      if (ignore) return;
      const next = {};
                ...
                <div className="p-4 text-white" style={{background:'var(--blue-gradient)'}}>
                  ...
                  {ratings[doc.doctorId] && (
                    <div className="mt-2 small text-white">
                      {'⭐'.repeat(Math.round(ratings[doc.doctorId].average))} {ratings[doc.doctorId].average.toFixed(1)}
                      <span className="opacity-75"> ({ratings[doc.doctorId].count} review{ratings[doc.doctorId].count === 1 ? '' : 's'})</span>
                    </div>
                  )}
                </div>
